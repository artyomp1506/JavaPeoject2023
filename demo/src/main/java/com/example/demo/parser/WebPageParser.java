...
public class WebPageParser {
    ...
    public Book[] getBooksFromPage(){
        ...
        for(int i = 0; i < titles.length; i++){
            books.add(new Book(titles[i], authors[i],Float.parseFloat(prices[i])));
        }
        return books.toArray(Book[]::new);
    }

    public Book[] getBooksFromPageLabirint(){
        List<Book> books = new ArrayList<>();
        //String[] titles = getAttributes(page.substring(0), "<div class=\"product-title__head\">", '<');
        String[] titles = getAttributes(page.substring(0), KeyTitle[0], KeyTitle[1]);
        //String[] authors = getAttributes(page.substring(0), "<div class=\"product-title__author\">", '<');
        ...
        for(int i = 0; i < titles.length; i++){
            books.add(new Book(authors[i],titles[i], Float.parseFloat(prices[i])));
        }
        return books.toArray(Book[]::new);
    }
